...
The official AOTY site does not provide a public API, so this project uses [AlbumOfTheYearAPI](https://github.com/JahsiasWhite/AlbumOfTheYearAPI) and a small, custom Python helper to export your ratings for local use.
...
1) Run the data-gathering script
   ...
   - **Warning:** Some albums or artists may be skipped if their AOTY entries contain unusual characters or inconsistent formatting. Consider editing the output file to fix formatting issues before re-running.

2) (Optional) Obtain a Last.fm API key
	- The wallpaper queries Last.fm for album artwork and track listings. Obtain a personal API key at https://www.last.fm/api.
